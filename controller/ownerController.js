...
export const loginOwner = async ({ username, cid }) => {
    try {
        // console.log('Login attempt with:', { ...req.body, license_number: '***' });
        // const { username, cid } = req.body;
        console.log('Received username:', username, 'and CID:', cid);
        ...
        const query = `
            ...
            FROM business_owner 
            WHERE username = $1 AND cid = $2
        `;

        console.log('Executing query for username:', username);
        const result = await db.query(query, [username, cid]);
